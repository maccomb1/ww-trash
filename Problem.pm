...
sub siblings {
	...
	my $nav_url = $r->location . $urlpath->parent->path;

	print CGI::start_div({class=>"info-box", id=>"fisheye", style=>"background-color:initial; border:0px; padding-right:0px"});
	print CGI::h2({style=>"border-radius:5px; margin-right:6px;"},  CGI::a({style=>"color:white;", href=>$nav_url }, $r->maketext("&uarr; Problem List"))  );
	...
	if($num_of_problems>0 and $r->ce->{pg}->{options}->{enableProgressBar}){
	    ...
	    my $progress_bar = CGI::start_div({-style=>"border-radius:0px;", -class=>"progress-bar set-id-tooltip",
	...
	print CGI::start_ul({class=>"problem-list", style=>"padding-right: 0px"});
...
sub nav {
	...
	return "";
	#	return $self->navMacro($args, $tail, @links); RYAN
}
...
sub output_score_summary{
	...
	unless (defined( $pg->{state}->{state_summary_msg}) and $pg->{state}->{state_summary_msg}=~/\S/) {
		...
		my $notCountedMessage = ($problem->value) ? $r->maketext("<strong>Problem Value</strong>: [_1] point(s). ", $problem->value) : $r->maketext("<strong>Problem Value</strong>: 0 points. ");
		print join("",
			...
			$r->maketext("<strong>Problem Score</strong>: [_1]. ",$lastScore),
			$setClosed ? $setClosedMessage : $r->maketext("<strong>Attempts Remaining:</strong> [negquant,_1,Unlimited,attempt,attempts].",$attemptsLeft) 
		);
	}else {
...
sub output_summary{
	...
	if (defined($pg->{flags}->{showPartialCorrectAnswers}) and ($pg->{flags}->{showPartialCorrectAnswers} >= 0 and $submitAnswers) ) {
	    ...
	    print CGI::div({class=>'alert', style=>'margin-bottom: 0px'},$r->maketext("ANSWERS ONLY CHECKED -- ANSWERS NOT RECORDED"));
	  ...
	  # print this if user previewed answers
	    print CGI::div({class=>'alert', style=>'margin-bottom: 0px'},$r->maketext("PREVIEW ONLY -- ANSWERS NOT RECORDED")),CGI::br(),$self->attemptResults($pg, 1, 0, 0, 0, 1);
	    # show attempt answers
