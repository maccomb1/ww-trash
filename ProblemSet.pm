...
#  if ($enable_reduced_scoring && before($set->reduced_scoring_date)) {
#    $title .= ' - '.$r->maketext("Due for 100% on [_1]", $self->formatDateTime($set->reduced_scoring_date,undef,$r->ce->{studentDateDisplayFormat}));
#  } elsif ($set->due_date) {
#    $title .= ' - '.$r->maketext("Was due for 100% on [_1]", 
...
print CGI::style({}, $r->maketext("#fisheye { display:none; }") );
...
print CGI::style({}, $r->maketext(" .nav_button {display:none;}") );
...
# print CGI::div({-class=>"problem_set_options", -style=>"display:block; float:right;"}, CGI::a({href=>$hardcopyURL}, $r->maketext("Download PDF or TeX Hardcopy for Current Set")));
...
my $reduced_scoring_date_disp = $self->formatDateTime($set->reduced_scoring_date);
my $dueDate = $set->due_date();
my $dueDate_disp = $self->formatDateTime($set->due_date());
...
if ($enable_reduced_scoring) {
	...
	if (before($reduced_scoring_date)) {
		print CGI::div({style=>"margin-top:-18px; font-weight:bold; font-size:larger;"}, $r->maketext("Due for full credit on [_1].", $reduced_scoring_date_disp));  
		print CGI::div({style=>"margin-bottom:22px; font-weight:bold;", class=>"ResultsAlert"}, $r->maketext("After the due date above all work counts for [_1]% of its value until [_2].", $reducedScoringPerCent, $dueDate_disp));
	} elsif (between($reduced_scoring_date,$set->due_date())) {
		print CGI::div({style=>"margin-top:-18px; font-weight:bold; font-size:larger;"}, $r->maketext("Was due for full credit on [_1].", $reduced_scoring_date_disp));  
		print CGI::div({style=>"margin-bottom:22px; font-weight:bold;", class=>"ResultsAlert"},$r->maketext("This set is in its reduced credit period.  All work counts for [_1]% of its value until [_2].",$reducedScoringPerCent, $dueDate_disp));
	} else {
		print CGI::div({style=>"margin-top:-18px; margin-bottom:22px; font-weight:bold; margin-bottom:8px;"},$r->maketext("This set was due on [_1]. Reduced credit period ended on [_2] during which work counted for [_3]% of its value.",$beginReducedScoringPeriod,$dueDate_disp,$reducedScoringPerCent));
	}
} else {
	if (before($dueDate)) {
		print CGI::div({style=>"margin-top:-18px; margin-bottom:22px; font-weight:bold; font-size:larger;"}, $r->maketext("Due on [_1].", $dueDate_disp));  
	} else {
		print CGI::div({style=>"margin-top:-18px; margin-bottom:22px; font-weight:bold; font-size:larger;"}, $r->maketext("Was due on [_1].", $dueDate_disp));
	}
...
# This table now contains a summary, a caption, and scope variables for the columns.
    print CGI::start_table({-class=>"table table-striped"});

    my  $AdjustedStatusPopover = " ".CGI::a({class=>'help-popup',href=>'#', 'data-content'=>$r->maketext('The adjusted status of a problem is the larger of the problem\'s status and the weighted average of the status of those problems which count towards the parent grade.')  ,'data-placement'=>'top', 'data-toggle'=>'popover'},'⍰');
...
print CGI::div({-class=>"problem_set_options", -style=>"display:block; float:right;"}, CGI::a({href=>$hardcopyURL}, $r->maketext("Download PDF or TeX Hardcopy for Current Set"))).CGI::br();
...
# if the problem is restricted we show that it exists but its greyed out
if ($isJitarSet && !$authz->hasPermissions($problem->user_id, "view_unopened_sets") && is_jitar_problem_closed($db, $ce, $problem->user_id, $setID, $problemID)) {
	...
	my $my_max = (($problem->max_attempts||-1) < 0) #a blank yields 'infinite' because it evaluates as false with out giving warnings about comparing non-numbers
	? 0 : $problem->max_attempts;
	...
	if ($my_max == 47 || $my_max == 999) {
		$bonus_text .= 'Challenging ';	
	}
    $interactive = CGI::a({-style=>"font-weight:bold;", -href=>$interactiveURL,-class=>$linkClasses}, $r->maketext($bonus_text . "Problem [_1]",$problemNumber));
}
