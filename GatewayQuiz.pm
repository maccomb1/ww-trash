...
###
	...
	if ( $can{recordAnswersNextTime} ) {
		...
		#    submission
		if ( $set->attempts_per_version > 1 ) {
			print CGI::div({-class=>"alert alert-info", -style=>"text-align:center; font-weight:bold; margin-bottom:0px"}, CGI::em($r->maketext("You have [_1] grading submission(s) remaining on this assignment.",$numLeft)));
		...
		foreach my $i ( 0 .. $#pg_results ) {
			...
			if ( $i >= $startProb && $i <= $endProb ) { 
				...
				if ($pg->{flags}->{showPartialCorrectAnswers}>=0 && $submitAnswers){
					...
					$recordMessage = 
					    CGI::span({class=>"alert alert-info", style=>"font-weight:bold; padding:0px 12px;"},
						      $r->maketext("SAVE and PREVIEW ONLY -- ANSWERS NOT SUBMITTED FOR GRADING"));
				...
				print CGI::h2({style=>"margin-bottom:4px"},$r->maketext("Problem [_1].",$problemNumber)), 
					$recordMessage;
				print CGI::start_div({class=>"gwProblem"});
				...
				CGI::p($pg->{result}->{msg} ? 
				       ...
				       CGI::i($pg->{result}->{msg}));
				print CGI::p({class=>"gwPreview", style=>"text-align:center;"}, 
					     CGI::a({-href=>"$jsprevlink", class=>"btn btn-success", style=>"font-weight:bold;"}, 
						    $r->maketext("SAVE and preview answers")));
...
#				-checked => $will{showCorrectAnswers},
		...
		print CGI::p( CGI::submit( -name=>"previewAnswers", 
					   -label=>$r->maketext("Preview Assignment") ),
			      ($can{recordAnswersNextTime} ? 
			       CGI::submit( -name=>"submitAnswers",
					    -label=>$r->maketext("Grade Assignment") ) : " "),
